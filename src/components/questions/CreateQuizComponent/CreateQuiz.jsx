...
const CreateQuiz = () => {
  ...
  const { linkImg, upLoadSuccess, newQuiz, creatQuizSuccess, isLoading } =
    useSelector((store) => store.createQuiz);
  ...
  useEffect(() => {
    dispatch(fetchQuizbyId({ token: user.access_token, id: newQuiz.id }));
    if (creatQuizSuccess) {
